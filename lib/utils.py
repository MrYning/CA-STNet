...
def compute_loss(net,dataloader,risk_mask, trans_file,
    ...
    for feature,feature_small,target_time,label in dataloader:
        trans,feature,feature_small,target_time,label = trans.to(device),feature.to(device),feature_small.to(device),target_time.to(device),label.to(device)
        l = mask_loss(net(feature,feature_small,target_time,trans),label,risk_mask,data_type)
...
def predict_and_evaluate(net,dataloader,risk_mask, trans_file,
    ...
    for feature,feature_small,target_time,label in dataloader:
        trans,feature,feature_small,target_time,label = trans.to(device),feature.to(device),feature_small.to(device),target_time.to(device),label.to(device)
        prediction_list.append(net(feature,feature_small,target_time,trans).cpu().numpy())
